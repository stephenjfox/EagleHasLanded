 ...
 */
class HashProblemSolver/*val mdHashMan = HashingUtilities.getMessageDigest()
    ...
    private val hashMatcher: (String) -> Boolean = { s ->
        val preppedMessageDigest = HashingUtilities.getPreppedMessageDigest(s)
        val toString = HashingUtilities.getStringBuilderFromMessageDigest(preppedMessageDigest).toString()
        val equals = hashToFind == toString
        if (equals) {
    ...
    fun runIt() {
        ...
        val amountPerBatch = fourLettersList.size / workers.size.coerceAtLeast(1)
        val partitionsOfFour = fourLettersList.chunked(amountPerBatch)
...
//            ConsoleLogger.writeLine("Main thread was interrupted. Time to check mail", ConsoleColor.CYAN)
            println("Main thread was interrupted. Time to check mail")
